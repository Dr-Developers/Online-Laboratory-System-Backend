...
const getReportsByFilter = async (request, response) => {
	try {
		let { searchFilter } = request.body;
		let reportViewModel = [];
		if (searchFilter) {
			reportViewModel = await Report.find({
				firstName: { $regex: searchFilter, $options: "i" },
			});
		} else {
			reportViewModel = await Report.find();
		}
		response.json(reportViewModel);
	} catch (error) {}
}; //1
...
const deleteReport = async (req, res) => {
	...
	try {
		...
		if (!report) {
			...
			const deletedReport = await Report.findByIdAndDelete(reportID); // if found delete report
